...
Once the program is built, run `deploy` in the terminal to deploy the program to the cluster (devnet by default).c
...
You can then view the transaction logs by running the `solana confirm -v` command and specifying the transaction hash from the test output:

```shell filename="Terminal" copy
solana confirm -v [TxHash]
```

Alternatively, the transaction can be viewed on [Solana Explorer](https://explorer.solana.com/?cluster=devnet).
...
Click the `Import` button and enter a project name to add it to your list of projects in Solana Playground.
