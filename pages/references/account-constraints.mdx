...
Checks the given account is mutable. This makes Anchor persist any state changes. Custom errors are supported via @.
...
Checks that the given account is a PDA derived from the currently executing program, the seeds, and if provided, the bump. If not provided, Anchor uses the canonical bump. Add `seeds::program = <expr>` to derive the PDA from a different program than the currently executing one.
